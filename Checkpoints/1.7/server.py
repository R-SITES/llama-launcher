...
- Keep the message concise but informative."""
    ...
    def _serve_launch(self):
        """Launch llama-server in background (nohup) and close terminal after launch."""
        from urllib.parse import urlparse, parse_qs
        import subprocess
        import os
        ...
        # Build the full launch with oneAPI env setup + nohup for detaching
        full_cmd = f"source /opt/intel/oneapi/setvars.sh --force && nohup {cmd_str} > /dev/null 2>&1 & disown; exit"

        try:
            # Use xdg-terminal-exec to show a brief flash (optional, user can close manually)
            ...
            # Store PID file so /api/exit can kill the terminal if user wants
            pid_file = os.path.expanduser("~/llama-term-pid.tmp")
            ...
            self._json({
                "output": f"llama-server launched in background: {cmd_str}",
                "success": True,
                "pid_file": pid_file
            })
            ...
            try:
                ...
                self._json({"output": f"launched (gnome-terminal): {cmd_str}", "success": True})
            except FileNotFoundError:
                # Last resort: xterm
                subprocess.Popen(
                    ["xterm", "-e", full_cmd],
                ...
                self._json({"output": f"launched (xterm): {cmd_str}", "success": True})
