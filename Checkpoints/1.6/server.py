    ...
    def _serve_status(self):
        ...
        try:
            ...
            if result.stdout.strip():
                ...
                # Try to get stats from the server if it has an API endpoint
                # Most llama-server instances don't expose stats by default
                # For now, we'll poll the terminal output or use a simple heuristic
                # TODO: Add --stats or --api to llama-server for real throughput
                
                # Alternative: check if we can read from a socket file or named pipe
                # For now, assume unknown throughput if server is running
                throughput = "?"
